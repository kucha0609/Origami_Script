...
if (app.documents.length == 0) {//当前不在欢迎界面
    ...
    //Global:定义全局变量--------------------------------------------------
        ...
        var G_Black = new CMYKColor();//黑色
            ...
            G_Black.black =100;
        var G_White = new CMYKColor();//白色
            G_White.cyan = G_White.magenta = G_White.yellow = G_White.black =0
    ...
    //Global:定义窗口和按钮功能------------------------------------------
        ...
        var AtDF = SubfPnl.add('checkbox', undefined, '深色面变灰');
            AtDF.value = false ;//不勾选
        var AtLF = SubfPnl.add('checkbox', undefined, '浅色面变白');
            AtLF.value = false ;//不勾选
        ...
        var At02 = SubfPnl.add('checkbox', undefined, '删除旧的画板');
            At02.value = true;//勾选
        var At03 = SubfPnl.add('checkbox', undefined, '关闭层Page+Defps+0');
            At03.value = true;//勾选
        ...
        AtDF.size = AtLF.size = At01.size = At02.size = At03.size = [200, 20]
    ...
    //~function_01------------------------------------------------------
        ...
        function Sort_AllLay(Flag){//~~Adjust layer order (Alphabetized)  
            ...
            for (i = 0; i < DocLayers.length; i++) {
                ...
                if(DocLayers[i].visible){
                    DocLayers[i].locked = false;//解锁图层
                    visibleLayers.push(DocLayers[i]);
                }//图层可见则收集
            }
            if(!visibleLayers.length == 0){
        ...
        function Chg_ObjType() {//~~Change the color in layer (FillColor and StrokeColor)
            ...
            //仅透明度
            ChgAll_ObjType(["0"], false, true, true, false, 100, 100);
            ...
            if (AtDF.value) {
                ...
                ChgAll_ObjType(["D-14-FaceDark"], false, true, true, false, 100, 100);
            }
            if (AtLF.value) {
                ChgAll_ObjType(["D-15-FaceLight"], G_White, true, true, false, 100, 100);  
            } else {
                ChgAll_ObjType(["D-15-FaceLight"], false, true, true, false, 100, 100);
            }
