...
function TextFun(MdeSlt, CurIdx, X, Y, T, N) {
    ...
    for (var i = 0; i < X * Y; i++) {
        ...
        var sb = StoryBuilder.create();
            ...
            sb.applyGlyphDelta(StoryDelta.createGlyphDouble(GlyphAttDoubleType.Height, pt2px(T)));// 字号（px，等同于 pt 在对应 dpi 下）
            ...
            sb.addText(NamSTR + ".");//文字内容

        var def = ArtTextNodeDefinition.createFromStoryBuilder({ x: dx, y: dy }, sb);
            def.userDescription = "StepN_" + NamSTR; // ← 创建时直接设置名称
    ...
    CurDoc.executeCommand(acnBuilder.createCommand(false, NodeChildType.Main));//andSelect
    return StaNum;
}//在指定画板生成递增序号
function MainFun() {
    var CurDoc  = Document.current;
